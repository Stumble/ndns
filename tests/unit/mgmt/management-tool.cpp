...
public:
    ...
    , m_dbMgr(TEST_DATABASE.string().c_str())
  {
    ...
    Certificate rootDkeyCert = createCertificate(m_keyChain, otherDskKey, otherKskKey, "CERT");
  ...
  findCertFromIdentity(const Name& identityName, const Name& certName)
  {
    ...
    throw Error("Certificate not found in keyChain");
    return rtn;
  }
...
BOOST_AUTO_TEST_CASE(AddRrset)
{
  ...
  rf.checkZoneKey();
  Rrset rrset1 = rf.generateNsRrset("/l1", 7654, ttl2, Link::DelegationSet());
  ...
  Rrset rrset3 = rf.generateNsRrset("/l1/l2/l3", 7654, ttl2, Link::DelegationSet());
...
BOOST_AUTO_TEST_CASE(AddMultiLevelLabelRrset)
{
  ...
  Rrset rrset1 = rf.generateNsRrset(labelName, 7654, ttl, Link::DelegationSet());
  ...
  Rrset shorterNs = rf.generateNsRrset("/l1/l2", 7654, ttl, Link::DelegationSet());
  BOOST_CHECK_THROW(m_tool.addMultiLevelLabelRrset(shorterNs, rf, ttl),
                    ndns::ManagementTool::Error);

  // insert a longer NS, when there is already a shorter NS
  Rrset longerNs = rf.generateNsRrset("/l1/l2/l3/l4", 7654, ttl, Link::DelegationSet());
  ...
  Rrset smallerNs = rf.generateNsRrset("/k1/k2", 7654, ttl, Link::DelegationSet());
...
BOOST_AUTO_TEST_CASE(AddRrSetDskCert)
{
  ...
  // Rrset rrset = findRrSet(zone, getLabel(zone, fullName), label::CERT_RR_TYPE);
  // TODO: check if child zone's d-key could be inserted
  // BOOST_CHECK_NO_THROW(findRrSet(parentZone, getLabel(zone, ksk), label::CERT_RR_TYPE));
}
...
BOOST_AUTO_TEST_CASE(AddRrSetDskCertFormat)
{
  ...
  // raw
  dsk = m_keyChain.createKey(id);
  dskCert = dsk.getDefaultCertificate();
...
BOOST_AUTO_TEST_CASE(ListAllZones)
{
  ...
  Name rootDskName = getCertificate(m_keyChain, "/NDNS/", rootDsk).getKeyName();
  Name otherDskName = getCertificate(m_keyChain, "/ndns-test/NDNS/", otherDsk).getKeyName();
